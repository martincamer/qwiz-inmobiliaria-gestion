...
import { User, MapPin, CreditCard, FileText, Settings, Save, X, Edit } from 'lucide-react';

const EditOwnerModal = ({ open, onOpenChange, owner, onSuccess }) => {
  const { updateOwner, isLoading } = useOwners();
  ...
  useEffect(() => {
    if (owner && open) {
      setFormData({
        ...
        numeroIdentificacion: owner.numeroIdentificacion || '',
        tipoIdentificacion: owner.tipoIdentificacion || 'DNI',
        direccion: {
          calle: owner.direccion?.calle || '',
          numero: owner.direccion?.numero || '',
          piso: owner.direccion?.piso || '',
          departamento: owner.direccion?.departamento || '',
          ciudad: owner.direccion?.ciudad || '',
          provincia: owner.direccion?.provincia || '',
          codigoPostal: owner.direccion?.codigoPostal || ''
        },
        informacionBancaria: {
          banco: owner.informacionBancaria?.banco || '',
          tipoCuenta: owner.informacionBancaria?.tipoCuenta || 'CAJA_AHORRO',
          numeroCuenta: owner.informacionBancaria?.numeroCuenta || '',
          cbu: owner.informacionBancaria?.cbu || '',
          alias: owner.informacionBancaria?.alias || ''
        },
        informacionFiscal: {
          condicionIva: owner.informacionFiscal?.condicionIva || 'CONSUMIDOR_FINAL',
          ingresosBrutos: owner.informacionFiscal?.ingresosBrutos || ''
        },
        notas: owner.notas || '',
        configuracionNotificaciones: {
          email: owner.configuracionNotificaciones?.email ?? true,
          sms: owner.configuracionNotificaciones?.sms ?? false,
          whatsapp: owner.configuracionNotificaciones?.whatsapp ?? false
        }
      });
      setErrors({});
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!owner?._id) {
      toast.error('Error: No se encontró el ID del propietario');
      return;
    }

    try {
      const result = await updateOwner(owner._id, formData);
      ...
      if (result.success) {
        ...
        toast.error(result.error || 'Error al actualizar propietario');
      }
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Modifique la información del propietario {owner.nombre} {owner.apellido}. Los campos marcados con * son obligatorios.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            ...
            <Button type="submit" disabled={isLoading}>
              ...
              {isLoading ? 'Actualizando...' : 'Actualizar Propietario'}
            </Button>
          </div>
        </form>
      </DialogContent>
